...
export const GetBlogDescription=(count)=>async(dispatch)=>{
    try{
        const resp=await $host.get("api/blog/getDescription",{count});
        if(resp.data.status==200){
            dispatch({type:blogActionTypes.FETCH_GET_DESCRIPTION,payload:resp.status.res});
        }
    }catch(err){
        console.log(err);
    }
